...
float *entity_re_Vec, *entity_im_Vec, *relation_re_Vec, *relation_im_Vec;
...
void init() {
    ...
    fclose(fin);
    printf("%d\n",relationTotal);
    relation_re_Vec = (float *)calloc(relationTotal * dimensionR, sizeof(float));
    relation_im_Vec = (float *)calloc(relationTotal * dimensionR, sizeof(float));
    fin = fopen("../data/FB15K/entity2id.txt", "r");
    tmp = fscanf(fin, "%d", &entityTotal);
    fclose(fin);
    printf("%d\n",entityTotal);
    entity_re_Vec = (float *)calloc(entityTotal * dimension, sizeof(float));
	entity_im_Vec = (float *)calloc(entityTotal * dimension, sizeof(float));
...
void prepare() {
    FILE *fin;
    int tmp;
    fin = fopen("entity_re2vec.vec", "r");
    for (int i = 0; i < entityTotal; i++) {
        int last = i * dimension;
        for (int j = 0; j < dimension; j++)
            tmp = fscanf(fin, "%f", &entity_re_Vec[last + j]);
    }
    fclose(fin);
    printf("successfully read entity_re_Vec\n");
	fin = fopen("entity_im2vec.vec", "r");
    for (int i = 0; i < entityTotal; i++) {
        int last = i * dimension;
        for (int j = 0; j < dimension; j++)
            tmp = fscanf(fin, "%f", &entity_im_Vec[last + j]);
    }
    fclose(fin);
    printf("successfully read entity_im_Vec\n");
    fin = fopen("relation_re2vec.vec", "r");
    for (int i = 0; i < relationTotal; i++) {
        int last = i * dimensionR;
        for (int j = 0; j < dimensionR; j++)
            tmp = fscanf(fin, "%f", &relation_re_Vec[last + j]);
    }
    fclose(fin);  
    printf("successfully read relation_re_Vec\n");
	fin = fopen("relation_im2vec.vec", "r");
    for (int i = 0; i < relationTotal; i++) {
        int last = i * dimensionR;
        for (int j = 0; j < dimensionR; j++)
            tmp = fscanf(fin, "%f", &relation_im_Vec[last + j]);
    }
    fclose(fin);  
    printf("successfully read relation_im_Vec\n");
}
...
float calc_sum(int e1, int e2, int rel) {
	...
	//return torch.sum(r_re * e_re_h * e_re_t + r_re * e_im_h * e_im_t + r_im * e_re_h * e_im_t - r_im * e_im_h * e_re_t,1,False)return torch.sum(r_re * e_re_h * e_re_t + r_re * e_im_h * e_im_t + r_im * e_re_h * e_im_t - r_im * e_im_h * e_re_t,1,False)
    for (int i = 0; i < dimensionR; i++)
        res+=relation_re_Vec[lastr+i]*entity_re_Vec[last1+i]*entity_re_Vec[last2+i]+relation_re_Vec[lastr+i]*entity_im_Vec[last1+i]*entity_im_Vec[last2+i]+relation_im_Vec[lastr+i]*entity_re_Vec[last1+i]*entity_im_Vec[last2+i]-relation_im_Vec[lastr+i]*entity_im_Vec[last1+i]*entity_re_Vec[last2+i];
    return -res;
}
