...
int Threads = 8;
int dimension = 50;
float *entityVec, *relationVec, *entityRelVec;
...
void init() {
    ...
    fclose(fin);
	printf("%d\n",relationTotal);
    relationVec = (float *)calloc(relationTotal * dimension * dimension, sizeof(float));
...
void* prepareMode(void *con) {
    ...
    for (int i = lef; i < rig; i++) {
        for (int j = 0; j < relationTotal; j++) {
            int last = i * relationTotal * dimension + j * dimension;
            for (int k = 0; k < dimension; k++)
                for (int kk = 0; kk < dimension; kk++)
                    entityRelVec[last + k] += relationVec[j * dimension * dimension + k * dimension + kk] * entityVec[i * dimension + kk];
        }
    }
}
...
void prepare() {
	...
	*/
    ...
    for (int i = 0; i < relationTotal; i++)
            for (int j = 0; j < dimension; j++)
                for (int k = 0; k < dimension; k++)
                    tmp = fscanf(fin, "%f", &relationVec[i * dimension * dimension + j*dimension + k]);
    fclose(fin);
    printf("success relation\n");
...
float calc_sum(int e1, int e2, int rel) {
	...
	*/
	float res = 0;
	int last = e2 * relationTotal * dimension + rel * dimension;
	for (int i = 0; i < dimension; i++){
		res += entityVec[e1 * dimension + i] * entityRelVec[last + i];
	}
	return -res;
}
